...
if __name__ == "__main__":
    ...
    while True:
        ...
        if user_input.lower() in ["quit", "exit", "bye"]:
            print("Assistant: Take care! Always consult a licensed healthcare professional for medical advice. Goodbye! 👋")
            break

        # 🔒 Domain Restriction
        if not is_medical_topic(user_input):
            print("Assistant: I'm designed to assist with health and wellness topics only. Please ask about symptoms, conditions, nutrition, or general medical guidance.")
        ...
        if detect_urgency(user_input):
            print("Assistant: 🚨 If you are experiencing a medical emergency, please call your local emergency number (e.g., 911) or go to the nearest hospital immediately.\n")

        # 🤖 Generate Response
        response = rag_bot.generate(user_input)
